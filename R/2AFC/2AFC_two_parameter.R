#
# Simulation of a 2AFC task with two parameters.
# Probability of a correct response is...
# 
#   P(R = 1) = pnorm((s / theta_1)^theta_2 / sqrt(2))
#
# ...where theta_1 is taken as the threshold and theta_2
# models non-linearity in transduction (of the physical 
# signal into internal).
#
...
prior = function(theta){
  dgamma(theta[,1], 2, 0.5) * dgamma(theta[,2], 10, 10)
}

likelihood = function(y, theta){
  p_corr = pnorm((y[1,1] / theta[,1])^theta[,2] / sqrt(2))
...
for(i in 1:n_simulations){
  ...
  r = rbinom(n_trials, 1, 
     pnorm((s / gen_theta[1,1])^gen_theta[1,2] / sqrt(2)))
  y = cbind(s, r)
